...
class Deblurring(H_functions):
    ...
    def __init__(self, kernel, channels, img_dim, device, ZERO = 1e-3):
        self.img_dim = img_dim
        self.channels = channels
        self.length = channels * img_dim ** 2
        #build 1D conv matrix
        ...
        self.all_singulars = torch.unsqueeze(self.singulars(), 0)
    ...
    def transpose(self, vec):
        vec = self.Ut(vec)
        vec = self.singulars() * vec
        vec = self.V(vec)
        vec = vec.reshape(vec.shape[0],3,256,256)
        return vec
        
    def forward(self, vec):
        return self.U(self.singulars() * self.Vt(vec)).reshape(vec.shape[0], 3, 256, 256)
